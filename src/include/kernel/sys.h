...
typedef struct task_struct_s {
    ...
    tsk_id_t private_tid;
    vfs_handle_bucket_t* handle_buckets;
    atomic_long_t t_lfd; /// Last fd value
    queue_t* t_recyc_fds; /// recycled fds
    task_spin_lock t_lock; /// task scope lock

    size_t count_each_bucket;
